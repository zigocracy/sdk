...
class SyntaxStreamBuilder {
	...
	private val diagnostics: MutableMap<Int, List<TokenDiagnostic>> = mutableMapOf()
	 ...
	 */
	fun addToken(kind: TokenKind, width: CodeUnits, diagnostics: List<TokenDiagnostic> = emptyList()) {
		val currentIndex = events.size
		events.add(TokenEvent(kind, width))
		if (diagnostics.isNotEmpty()) {
			this.diagnostics[currentIndex] = diagnostics
		}
	}
	...
	fun build() = SyntaxStream(events, diagnostics)
