...
import net.landless_city.zigocracy.zig.scanner.util.NumberAccumulator
import net.landless_city.zigocracy.zig.scanner.util.NumberBase
import net.landless_city.zigocracy.zig.scanner.util.isZigDecimalDigit
import net.landless_city.zigocracy.zig.shared.DiagnosticCode
import net.landless_city.zigocracy.zig.syntax.TokenKind

object NumberScanner : TokenScanner {
	override fun scan(reader: TextReader): ScanResult {
		...
		if (base != NumberBase.Decimal) {
			...
			if (nextChar == null || !base.isValidDigit(nextChar)) {
				localDiagnostics.add(TokenDiagnostic(DiagnosticCode.NumberError.MissingDigitAfterBase, relativeStart = 0, width = width))
		...
		return when (val next = reader.peekChar(1)) {
			...
			'.', 'e', 'E', null -> NumberBase.Decimal to 1
				...
				if (next.isZigDecimalDigit()) {
					diagnostics.add(TokenDiagnostic(DiagnosticCode.NumberError.LeadingZero, relativeStart = 0, width = 2))
				}
	...
	private fun validateNumberLiteral(text: String, base: NumberBase, diagnostics: MutableList<TokenDiagnostic>) {
		...
		if (text.last() == '_') {
			diagnostics.add(
				TokenDiagnostic(
					code = DiagnosticCode.NumberError.MalformedUnderscore, relativeStart = text.length - 1, width = 1
				)
			)
		}
		...
		for (i in startOffset until text.length) {
			...
			if (c == '.') {
				if (i > 0 && text[i - 1] == '_') {
					diagnostics.add(TokenDiagnostic(DiagnosticCode.NumberError.MalformedUnderscore, relativeStart = i - 1, width = 1))
				}
			...
			if (!isValid) {
				diagnostics.add(
					TokenDiagnostic(
						code = DiagnosticCode.NumberError.InvalidDigit, relativeStart = i, width = 1
					)
				)
			}
		}
