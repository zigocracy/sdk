 ...
 */
object CharScanner : TokenScanner {
		...
		while (true) {
			...
			when {
				...
				c == null || c.isZigVerticalWhitespace() -> {
					diagnostics.add(TokenDiagnostic(DiagnosticCode.CharError.UnterminatedChar, 0, width))
				...
				c == context.terminalChar -> {
					width++
					if (charCount == 0) {
						diagnostics.add(TokenDiagnostic(DiagnosticCode.CharError.EmptyCharLiteral, 0, width))
					}
					break
				}
